...
| Fecha | Estado | Nota |
|-------|--------|------|
| 2026-09-01 | implementado | PR #32 mergeado. 92 tests en el API y 347 en el móvil. Tres pasadas del `code-reviewer`, las dos primeras con GRAVE, y las dos sobre lo mismo: decidir dos veces sin señal dejaba en el servidor la decisión que la persona descartó. De ahí salieron las dos piezas que sobreviven al spec — **el turno de la bandeja**, que vuelve un solo acto el ciclo armar-enviar-aplicar del push, y **`expectedStatus`**, que separa corregirse de chocar. Probado en el teléfono: de ahí el copy de la hoja de rechazo y el botón que se partía en dos líneas. |
