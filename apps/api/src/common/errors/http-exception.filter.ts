...
const DB_ERROR_SIGNATURES: { match: string; code: ErrorCode; status: number; message: string }[] = [
  ...
  { match: 'uq_membership_single_owner', code: 'OWNER_ALREADY_EXISTS', status: 409,
    message: 'La empresa ya tiene un OWNER activo' },
  { match: 'membership_company_id_user_id_key', code: 'CONTACT_ALREADY_MEMBER', status: 409,
    message: 'Esa persona ya es parte de la empresa' },
  { match: 'app_user_needs_contact', code: 'VALIDATION_FAILED', status: 400,
    message: 'Hace falta un correo o un teléfono' },
  { match: 'crew_member_no_overlap', code: 'CREW_MEMBER_OVERLAP', status: 409,
